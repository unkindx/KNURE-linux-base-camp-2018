#include <linux/module.h>
#include <linux/kernel.h>
#include <linux/proc_fs.h>
#include <linux/sched.h>
#include <asm/uaccess.h>
#include <linux/slab.h>

#include <linux/timer.h>


MODULE_LICENSE("Dual BSD/GPL");
MODULE_AUTHOR("Dyomin Denis");
MODULE_DESCRIPTION("Procfs read");
MODULE_VERSION("0.1");


#define MODULE_TAG      "Fibonacci module "
#define PROC_DIRECTORY  "Fibonacci"
#define PROC_FILENAME   "Fibonacci"


//static char *proc_buffer;

//struct timer_list Tim;

static uint num1 = 0, num2 = 1;

static struct proc_dir_entry *proc_dir;
static struct proc_dir_entry *proc_file;

static int example_read(struct file *file_p, char __user *buffer, size_t length, loff_t *offset);

static struct file_operations proc_fops = {
    .read  = example_read,
    //.write = example_write,
};

/*
static void timer_func(struct timer_list *t)
{
	//generate element of Fibonacci sequence

    mod_timer(&Tim, jiffies + HZ);
}
*/

/*
static int create_buffer(void)
{
    proc_buffer = kmalloc(BUFFER_SIZE, GFP_KERNEL);
    if (NULL == proc_buffer)
        return -ENOMEM;
    proc_msg_length = 0;

    return 0;
}

static void cleanup_buffer(void)
{
    if (proc_buffer) {
        kfree(proc_buffer);
        proc_buffer = NULL;
    }
}
*/

static int create_proc_example(void)
{
    proc_dir = proc_mkdir(PROC_DIRECTORY, NULL);
    if (NULL == proc_dir)
        return -EFAULT;

    proc_file = proc_create(PROC_FILENAME, S_IFREG | S_IRUGO | S_IWUGO, proc_dir, &proc_fops);
    if (NULL == proc_file)
        return -EFAULT;

    return 0;
}


static void cleanup_proc_example(void)
{
    if (proc_file)
    {
        remove_proc_entry(PROC_FILENAME, proc_dir);
        proc_file = NULL;
    }
    if (proc_dir)
    {
        remove_proc_entry(PROC_DIRECTORY, NULL);
        proc_dir = NULL;
    }
}


static int example_read(struct file *file_p, char __user *buffer, size_t length, loff_t *offset)
{
    uint i = num2;
    num2 = num1 + num2;
    num1 = i;

    printk(KERN_NOTICE MODULE_TAG "Next Fibonacci sequence is %u \n", num2);

    return 0;
}

/*
static int example_write(struct file *file_p, const char __user *buffer, size_t length, loff_t *offset)
{
    size_t msg_length;
    size_t left;

    if (length > BUFFER_SIZE)
    {
        printk(KERN_WARNING MODULE_TAG "reduse message length from %u to %u chars\n", length, BUFFER_SIZE);
        msg_length = BUFFER_SIZE;
    }
    else
        msg_length = length;

    left = raw_copy_from_user(proc_buffer, buffer, msg_length);

    proc_msg_length = msg_length - left;
    proc_msg_read_pos = 0;

    if (left)
        printk(KERN_ERR MODULE_TAG "failed to write %u from %u chars\n", left, msg_length);
    else
        printk(KERN_NOTICE MODULE_TAG "written %u chars\n", msg_length);

    return length;
}
*/

static int __init example_init(void)
{
    int err;

	//Tim.expires = jiffies + HZ;
	//Tim.function = timer_func;
	//add_timer(&Tim);

    //err = create_buffer();
    //if (err)
        //goto error;

    err = create_proc_example();
    if (err)
        goto error;

    printk(KERN_NOTICE MODULE_TAG "loaded\n");
    return 0;

error:
    printk(KERN_ERR MODULE_TAG "failed to load\n");
    cleanup_proc_example();
    //cleanup_buffer();
    return err;
}


static void __exit example_exit(void)
{
	//del_timer(&Tim);
    cleanup_proc_example();
    //cleanup_buffer();
    printk(KERN_NOTICE MODULE_TAG "exited\n");
}


module_init(example_init);
module_exit(example_exit);

